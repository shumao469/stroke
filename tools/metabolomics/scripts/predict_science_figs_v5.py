...
def load_xlsx_matrix(
    ...
    """
    Returns:
      X_df: samples x features
      y: labels (string)
      meta_feat: optional feature meta table (may be empty)
    Expected:
      - One sheet contains sample rows with label_col and numeric feature columns.
      - Optional sheet named 'feature_meta' with columns ['feature','name','class','pathway'] etc.
    """
    xls = pd.ExcelFile(xlsx_path)
    sheets = xls.sheet_names
    if sheet is None:
        # pick first non-meta sheet
        candidate = [s for s in sheets if s.lower() not in ("feature_meta", "meta", "metadata")]
        sheet = candidate[0] if candidate else sheets[0]
    df = pd.read_excel(xlsx_path, sheet_name=sheet)
    df.columns = [str(c).strip() for c in df.columns]

    if label_col not in df.columns:
        # heuristic
        for cand in ["group", "label", "y", "class", "Subtype", "subtype"]:
            if cand in df.columns:
                label_col = cand
                break
    if label_col not in df.columns:
        raise ValueError(f"Cannot find label column '{label_col}' in sheet '{sheet}'. Columns: {list(df.columns)[:10]}...")
    ...
    # Feature meta (optional)
    meta_feat = pd.DataFrame()
    for s in sheets:
        if s.lower() == "feature_meta":
            meta_feat = pd.read_excel(xlsx_path, sheet_name=s)
            meta_feat.columns = [str(c).strip() for c in meta_feat.columns]
            break
