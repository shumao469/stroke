...
# ==========================================
# 核心算法模块 (Core Tracking & Zone Engine)
# ==========================================
...
# ==========================================
# 高阶数据聚类分析与亚组横向比较 (Cohort Analysis)
# ==========================================

def smart_group_mapping(group_name, video_name):
    """精确提取大类 (Day) 与亚组标号 (如 01, 02)"""
    group_name, video_name = str(group_name), str(video_name)
    day = 'Day 0'
    if '前' in group_name or '光栓' in group_name or 'Day 0' in group_name: day = 'Day 0'
...
def plot_advanced_cohort_analysis(results, output_dir):
    ...
    # === 图2：亚组区域探索图 (Stacked Zone Profile) ===
    zone_records = []
    for r in results:
    ...
    # === 图3：雷达聚类画像 (Behavioral Fingerprint Radar Chart) ===
    ...
    # === 图4：微观阵发状态分布 (Violin Plots by DayGroup) ===
    bout_records = []
    for r in results:
    ...
    # === 图6：独立马尔可夫转移矩阵 (Individual Markov Heatmaps) ===
...
st.sidebar.info("请上传原视频 (.mp4) 与你手涂的蓝色蒙版图 (_ROI_debug.jpg)。系统会通过名字自动匹配。")
