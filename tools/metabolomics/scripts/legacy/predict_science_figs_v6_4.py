...
class SafeSelectFromModel(BaseEstimator, TransformerMixin):
    """Select features from a fitted model, but guarantee at least `min_features`.

    Works well for tiny-N high-P settings where L1 models may select zero features in some folds.
    Selection priority:
      1) non-zero coefficients / importances (|coef| > eps), if any
      2) otherwise take top `min_features` by |coef| (or feature_importances_)
      3) if still degenerate, fall back to top-variance features from X

    Parameters
    ----------
    estimator : sklearn estimator
        Must expose `coef_` or `feature_importances_` after fitting.
    min_features : int
        Minimum number of features to keep.
    eps : float
        Threshold for treating coefficients as non-zero.
    """

    def __init__(self, estimator, min_features: int = 20, eps: float = 1e-12):
    ...
    def fit(self, X, y):
        ...
        if hasattr(self.estimator_, "coef_"):
            ...
            imp = np.asarray(self.estimator_.feature_importances_)

        n_features = X.shape[1]
        if imp is None or imp.size != n_features:
            # fallback: variance
            imp = np.nanvar(X, axis=0)
            imp = np.where(np.isfinite(imp), imp, 0.0)

        # primary mask: non-zero
        mask = imp > self.eps
        if mask.sum() < 1:
            mask = np.ones(n_features, dtype=bool)

        # ensure at least min_features
        k = min(max(self.min_features, 1), n_features)
        if mask.sum() < k:
...
def oversample_train_only(
    ...
    Why this exists:
    - With tiny n (e.g., after dropping outliers), some outer-fold training splits may
      have only 1 sample in a class. kNN-based oversamplers (SMOTE/ADASYN) will fail.
    - We therefore:
        * Use SMOTE/ADASYN when feasible (min class count in *this training split* >= 2),
          with k dynamically chosen as min(5, min_cnt-1).
        * Otherwise, fall back to balanced noise augmentation (Gaussian jitter by feature std)
          if augment_n>0, so the pipeline keeps running and the sensitivity analysis is still
          "training-only augmented".
        * If augment_n==0, we skip oversampling and return X,y unchanged.

    Notes:
    - Noise sigma per feature is std(feature in training split) * noise_scale.
    - Balanced augmentation picks classes with inverse-frequency weights (helps minority).
    """
    if method == "none":
        return X_train, y_train
    ...
    # class counts in this training split
    vc = pd.Series(y_train).value_counts()
    min_cnt = int(vc.min()) if len(vc) else 0

    def _noise_augment_balanced(X: np.ndarray, y: np.ndarray, n_new: int) -> Tuple[np.ndarray, np.ndarray]:
        if n_new <= 0:
            return X, y
        rng = check_random_state(seed)
        ...
        std = np.nanstd(X0, axis=0)
        std = np.where(np.isfinite(std) & (std > 0), std, 1.0)

        classes = pd.unique(pd.Series(y0))
        counts = pd.Series(y0).value_counts().reindex(classes).values.astype(float)
        w = 1.0 / np.clip(counts, 1.0, None)
        w = w / w.sum()

        X_new = np.zeros((n_new, X0.shape[1]), dtype=float)
        y_new = np.zeros((n_new,), dtype=y0.dtype)

        for i in range(n_new):
            c = rng.choice(classes, p=w)
            idxs = np.flatnonzero(y0 == c)
            base_i = int(rng.choice(idxs))
            noise = rng.normal(loc=0.0, scale=std * float(noise_scale), size=X0.shape[1])
            X_new[i] = X0[base_i] + noise
            y_new[i] = c

        return np.vstack([X0, X_new]), np.concatenate([y0, y_new])

    if method in ("smote", "adasyn"):
        if min_cnt < 2:
            # Not enough samples per class in this training split
            if augment_n > 0:
                warnings.warn(
                    f"{method.upper()} skipped (min class count in this training split={min_cnt}). "
                    f"Falling back to noise augmentation (n_new={augment_n})."
                )
                return _noise_augment_balanced(X_train, y_train, int(augment_n))
            warnings.warn(
                f"{method.upper()} skipped (min class count in this training split={min_cnt}). "
                f"Use --oversample noise or increase samples per class."
            )
            return X_train, y_train

        if not _IMBLEARN_OK:
            raise RuntimeError(
                "imblearn is required for SMOTE/ADASYN. Please install imbalanced-learn "
                "or use --oversample noise/none."
            )

        k = int(max(1, min(5, min_cnt - 1)))
        if method == "smote":
...
def build_pipeline(seed: int, do_feature_select: bool, select_C: float = 0.5) -> Pipeline:
    ...
    if do_feature_select:
        ...
        steps.append(("select", SafeSelectFromModel(selector_model, min_features=20)))
...
# =========================
# C. Robustness diagnostics
# =========================

def compute_log2fc_mean_diff(X_df: pd.DataFrame, y_str: pd.Series, group_a: str, group_b: str) -> pd.Series:
    """Compute per-feature mean difference (A - B) on (log-)intensity scale.
    If data are log2-transformed, this corresponds to log2FC.
    """
    ya = (y_str.values == group_a)
    yb = (y_str.values == group_b)
    if ya.sum() < 1 or yb.sum() < 1:
        raise ValueError(f"Not enough samples for FC: {group_a} n={ya.sum()}, {group_b} n={yb.sum()}")
    ma = X_df.loc[ya].mean(axis=0, skipna=True)
    mb = X_df.loc[yb].mean(axis=0, skipna=True)
    fc = (ma - mb)
    fc.name = f"log2FC_{group_a}_vs_{group_b}"
    return fc


def _safe_kde_1d(x: np.ndarray, grid_n: int = 256):
    x = np.asarray(x, float)
    x = x[np.isfinite(x)]
    if x.size < 3:
        return None, None
    xs = np.linspace(np.min(x), np.max(x), grid_n)
    try:
        ...
        ys, edges = np.histogram(x, bins=20, density=True)
        xs = 0.5 * (edges[:-1] + edges[1:])
        return xs, ys


def _safe_kde_2d(x: np.ndarray, y: np.ndarray, grid_n: int = 128):
...
def plot_concordance_density_marginals(
    x: np.ndarray,
    y: np.ndarray,
    out_png: str,
    out_pdf: str | None = None,
    title: str = "log2FC concordance (density + marginals)",
    xlabel: str = "log2FC (full)",
    ylabel: str = "log2FC (sensitivity)",
    annotate_quadrants: bool = True,
    vline: float = 0.0,
    hline: float = 0.0,
    hex_gridsize: int = 35,
    add_kde_contour: bool = True,
):
    import matplotlib.pyplot as plt
    ...
    hb = ax_main.hexbin(x, y, gridsize=hex_gridsize, mincnt=1)
    cb = fig.colorbar(hb, ax=ax_main, fraction=0.046, pad=0.02)
    cb.set_label("count")

    lim = np.nanmax(np.abs(np.r_[x, y])) * 1.05 if x.size else 1.0
    ax_main.plot([-lim, lim], [-lim, lim], linewidth=1.0)
    ax_main.axvline(vline, linewidth=0.8)
    ax_main.axhline(hline, linewidth=0.8)

    if add_kde_contour:
        kde2 = _safe_kde_2d(x, y, grid_n=128)
        if kde2 is not None:
            X, Y, Z = kde2
            Zf = Z[np.isfinite(Z)]
            if Zf.size > 0:
                levels = np.quantile(Zf, [0.70, 0.85, 0.93])
                levels = np.unique(levels)
                levels = np.sort(levels)
                # matplotlib requires strictly increasing levels
                if levels.size >= 2 and np.all(np.diff(levels) > 0):
                    ax_main.contour(X, Y, Z, levels=levels, linewidths=1.0)

    ax_main.set_xlabel(xlabel)
    ax_main.set_ylabel(ylabel)
    ax_main.set_title(title)

    xs, xd = _safe_kde_1d(x)
    if xs is not None:
        ax_top.plot(xs, xd, linewidth=1.2)
    ax_top.set_xlim(ax_main.get_xlim())
    ax_top.axis("off")

    ys, yd = _safe_kde_1d(y)
    if ys is not None:
        ax_right.plot(yd, ys, linewidth=1.2)
    ax_right.set_ylim(ax_main.get_ylim())
    ax_right.axis("off")

    if annotate_quadrants and x.size:
        q1 = np.mean((x >= vline) & (y >= hline))
        q2 = np.mean((x <  vline) & (y >= hline))
        q3 = np.mean((x <  vline) & (y <  hline))
        q4 = np.mean((x >= vline) & (y <  hline))
        same = q1 + q3
        opp  = q2 + q4
        txt = (f"Q1(+,+): {q1*100:.1f}%\n"
               f"Q2(-,+): {q2*100:.1f}%\n"
               f"Q3(-,-): {q3*100:.1f}%\n"
               f"Q4(+,-): {q4*100:.1f}%\n"
               f"Same-sign: {same*100:.1f}% | Opp-sign: {opp*100:.1f}%")
...
def _macro_ovr_auc(y_int: np.ndarray, proba: np.ndarray, n_classes: int) -> float:
    aucs = []
    for c in range(n_classes):
        yt = (y_int == c).astype(int)
...
def compute_oof_proba_simple(X_df: pd.DataFrame, y_str: pd.Series, seed: int = 1, outer_splits: int = 4):
    """Compute OOF probabilities on full (non-augmented) data for influence diagnostics."""
    le = LabelEncoder()
    y_int = le.fit_transform(y_str.values)
    classes = list(le.classes_)
    n_classes = len(classes)
    minc = pd.Series(y_int).value_counts().min()
    k = min(outer_splits, int(minc)) if int(minc) >= 2 else 2
    cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=seed)

    X = X_df.values
    proba_oof = np.full((X.shape[0], n_classes), np.nan, dtype=float)
    ...
    for tr, te in cv.split(X, y_int):
        base.fit(X[tr], y_int[tr])
        proba_oof[te] = base.predict_proba(X[te])

    return proba_oof, y_int, classes


def compute_centroid_distance_pca(X_df: pd.DataFrame, y_str: pd.Series, n_components: int = 2) -> pd.DataFrame:
    Xs = StandardScaler(with_mean=True, with_std=True).fit_transform(X_df.values)
    pcs = PCA(n_components=n_components, random_state=0).fit_transform(Xs)
    df = pd.DataFrame(pcs, index=X_df.index, columns=[f"PC{i+1}" for i in range(n_components)])
    df["group"] = y_str.values
    cent = df.groupby("group")[["PC1", "PC2"]].mean()
    dists = []
    for idx, r in df.iterrows():
        c = cent.loc[r["group"]].values
        p = r[["PC1", "PC2"]].values.astype(float)
        dists.append(float(np.linalg.norm(p - c)))
    df["dist_to_centroid"] = dists
    return df


def plot_influence_auc_vs_distance(
    X_full: pd.DataFrame,
    y_full: pd.Series,
    out_png: str,
    out_pdf: str | None = None,
    highlight=("NC1", "NC3"),
    title="Influence diagnostic: Δmacro-AUROC vs distance-to-centroid",
    seed: int = 1,
    outer_splits: int = 4,
):
    # OOF on full data
    proba_oof, y_int, classes = compute_oof_proba_simple(X_full, y_full, seed=seed, outer_splits=outer_splits)
    n_classes = len(classes)
    base_auc = _macro_ovr_auc(y_int, proba_oof, n_classes)

    pca_df = compute_centroid_distance_pca(X_full, y_full)
    dist_map = pca_df["dist_to_centroid"].to_dict()

    rows = []
    ids = list(X_full.index.astype(str))
    for i, sid in enumerate(ids):
        m = np.ones(len(ids), dtype=bool)
        m[i] = False
        auc_wo = _macro_ovr_auc(y_int[m], proba_oof[m], n_classes)
        rows.append({
            "sample": sid,
            "group": str(y_full.iloc[i]),
            "delta_macroAUROC": float(auc_wo - base_auc),
            "dist_to_centroid": float(dist_map.get(sid, np.nan)),
        })
    rep = pd.DataFrame(rows).sort_values("delta_macroAUROC")

    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(7.2, 5.6))
    ax.axvline(0, linewidth=0.8)
    ax.scatter(rep["delta_macroAUROC"], rep["dist_to_centroid"], s=40)

    # label all points (small N)
    for _, r in rep.iterrows():
        ax.text(r["delta_macroAUROC"], r["dist_to_centroid"], r["sample"], fontsize=9,
                ha="left", va="center")

    # highlight
    for h in highlight:
        if h in set(rep["sample"].values):
            rh = rep[rep["sample"] == h].iloc[0]
            ax.scatter([rh["delta_macroAUROC"]], [rh["dist_to_centroid"]], s=90)

    ax.set_xlabel("Δ macro-AUROC after removing sample (OOF-based)")
    ax.set_ylabel("Distance to group centroid (PCA space)")
    ax.set_title(title)
...
def main():
    ...
    # ----- Load data
    X_full, y_full, meta_feat = load_xlsx_matrix(args.xlsx, label_col=args.label_col, id_col=args.id_col, sheet=args.sheet)

    # Main analysis data (may drop outliers)
    X_df, y_str = apply_drop_samples(X_full.copy(), y_full.copy(), args.drop)


    # basic cleanup
    X_df = X_df.loc[:, X_df.notna().sum(axis=0) >= 2]  # remove near-empty features

    # ----- C7: FC concordance (Full vs Sensitivity) as density + marginals
    # Only meaningful when args.drop is provided (e.g., dropping NC1/NC3).
    if len(args.drop) > 0:
        try:
            # Ensure the same feature set for FC computation
            feat_cols = [c for c in X_full.columns if c in X_df.columns]
            X_full2 = X_full.loc[:, feat_cols]
            X_sens2 = X_df.loc[:, feat_cols]

            # HS vs NC
            fc_full_hsnc = compute_log2fc_mean_diff(X_full2, y_full, "HS", "NC")
            fc_sens_hsnc = compute_log2fc_mean_diff(X_sens2, y_str, "HS", "NC")
            common = fc_full_hsnc.index.intersection(fc_sens_hsnc.index)
            plot_concordance_density_marginals(
                fc_full_hsnc.loc[common].values,
                fc_sens_hsnc.loc[common].values,
                out_png=os.path.join(args.out_dir, "ExtData_FigS1C_HSvsNC_FC_concordance_density.png"),
                out_pdf=os.path.join(args.out_dir, "ExtData_FigS1C_HSvsNC_FC_concordance_density.pdf"),
                xlabel="log2FC (Full HS vs NC)",
                ylabel="log2FC (Sensitivity HS vs NC)",
                title="log2FC concordance: Full vs Sensitivity (HS vs NC)",
                hex_gridsize=35,
                add_kde_contour=True,
            )

            # ZS vs HS
            fc_full_zshs = compute_log2fc_mean_diff(X_full2, y_full, "ZS", "HS")
            fc_sens_zshs = compute_log2fc_mean_diff(X_sens2, y_str, "ZS", "HS")
            common2 = fc_full_zshs.index.intersection(fc_sens_zshs.index)
            plot_concordance_density_marginals(
                fc_full_zshs.loc[common2].values,
                fc_sens_zshs.loc[common2].values,
                out_png=os.path.join(args.out_dir, "ExtData_FigS1C_ZSvsHS_FC_concordance_density.png"),
                out_pdf=os.path.join(args.out_dir, "ExtData_FigS1C_ZSvsHS_FC_concordance_density.pdf"),
                xlabel="log2FC (Full ZS vs HS)",
                ylabel="log2FC (Sensitivity ZS vs HS)",
                title="log2FC concordance: Full vs Sensitivity (ZS vs HS)",
                hex_gridsize=35,
                add_kde_contour=True,
            )
        except Exception as e:
            warnings.warn(f"FC concordance plot skipped due to: {e}")
    ...
    for fold, (tr, te) in enumerate(outer.split(X_df.values, y_int), start=1):
        ...
        if args.oversample in ("smote", "adasyn"):
            Xtr2, ytr2 = oversample_train_only(Xtr, ytr, method=args.oversample, seed=seed + fold, augment_n=args.augment_n, noise_scale=args.noise_scale)
        else:
            Xtr2, ytr2 = Xtr, ytr
    ...
    # ----- C8: Influence diagnostics on FULL dataset (auditable outlier defense)
    try:
        rep, base_auc = plot_influence_auc_vs_distance(
            X_full=X_full,
            y_full=y_full,
            out_png=os.path.join(args.out_dir, "Supp_Influence_deltaAUROC_vs_centroidDist.png"),
            out_pdf=os.path.join(args.out_dir, "Supp_Influence_deltaAUROC_vs_centroidDist.pdf"),
            highlight=("NC1", "NC3"),
            seed=seed,
            outer_splits=min(4, max(2, int(pd.Series(y_full).value_counts().min()))),
        )
        rep.to_csv(os.path.join(args.out_dir, "Supp_Influence_report.csv"), index=False)
    except Exception as e:
        warnings.warn(f"Influence diagnostics skipped due to: {e}")
